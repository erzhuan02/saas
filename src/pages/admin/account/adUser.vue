<template>
  <div id="adUser">
    ...
    <div class="queryForm">
      <el-form ref="queryForm" :model="queryForm" :rules="queryFormRules" inline label-width="auto">
        <el-form-item prop="searchName">
          <el-input v-model="queryForm.searchName" :placeholder="$t('placeholder.input1')+$t('common.companyName')+'/'+$t('common.userName')" maxlength="128" clearable></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <!-- 操作 -->
    <div class="CRUD">
      <el-button v-if="button.addUser" type="primary" icon="el-icon-plus" @click="handleDialogOpen('add')">{{ $t('button.add') }}</el-button>
      <el-button v-if="button.deleteUser" type="primary" icon="el-icon-delete" @click="deleteDialogVisible = true" :disabled="selection.length === 0">{{ $t('button.delete') }}</el-button>
    </div>
    <!-- 查询结果 表格 -->
    <div class="result clearfix" v-loading="listLoading">
      <el-table ref="resultTable" stripe border max-height="565" :data="list" tooltip-effect="dark" style="width: 100%;" @selection-change="handleSelectionChange" @sort-change="handleSortChange" highlight-current-row>
        ...
        <el-table-column sortable="custom" prop="companyName" :label="$t('common.companyName')" min-width="100"></el-table-column>
        <el-table-column sortable="custom" prop="userName" :label="$t('common.userName')"></el-table-column>
        <el-table-column sortable="custom" prop="lastLoginTime" :label="$t('account.lastLoginTime')" width="160"></el-table-column>
        <el-table-column sortable="custom" prop="createTime" :label="$t('account.createTime')" width="110"></el-table-column>
        ...
        <el-table-column prop="realName" :label="$t('account.contacts')"></el-table-column>
        ...
        <el-table-column :label="$t('common.operation')">
          <template slot-scope="scope">
            <el-button v-if="button.editUser" size="mini" type="primary" icon="el-icon-edit" @click="handleDialogOpen ('edit', scope.row)" plain>{{ $t('button.edit') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="$t('button.add')" :visible.sync="addDialogVisible" :close-on-click-modal="false" @closed="handleDialogClose('add')">
      <el-form :model="addForm" label-width="150px" :rules="addOrEditFormRules" ref="addForm" :validate-on-rule-change="false">
        ...
        <el-form-item :label="$t('account.role')" prop="roleId">
          <el-select v-model="addForm.roleId" multiple collapse-tags :placeholder="$t('placeholder.select')">
        ...
        <!-- <el-form-item :label="$t('account.role')" prop="roleId">
          <el-select v-model="addForm.roleId" :placeholder="$t('placeholder.select')">
            ...
            <el-option v-for="" :key="item.value" :label="item.text" :value="item.value"></el-option>
    ...
    <el-dialog :title="$t('button.edit')" :visible.sync="editDialogVisible" :close-on-click-modal="false" @closed="handleDialogClose('edit')">
      <el-form :model="editForm" label-width="140px" :rules="addOrEditFormRules" ref="editForm" :validate-on-rule-change="false">
        ...
        <el-form-item :label="$t('account.role')" prop="roleId">
          <el-select v-model="editForm.roleId" multiple collapse-tags :placeholder="$t('placeholder.select')" :disabled="editForm.adminUser">
            <el-option v-for="item in roleOptions" :key="item.roleId" :label="item.roleName" :value="item.roleId"></el-option>
            <el-option v-show="editForm.adminUser" label="超级管理员" value="1"></el-option>
          </el-select>
        </el-form-item>
        <!-- <el-form-item :label="$t('account.role')" prop="roleId">
        ...
        <el-form-item prop="emailAddr" :label="$t('common.email')">
          <div class="flex-hb">
            <el-input v-model.trim="editForm.emailAddr" :placeholder="$t('placeholder.input')" maxlength="50" :disabled="emailDisabledEdit" :style="{ width: emailDisabledEdit ? '70%' : '100%' }"></el-input>
        ...
        <el-form-item prop="emailCode" :label="$t('common.emailCode')">
          <div class="flex-hb">
            <el-input v-model.trim="editForm.emailCode" :placeholder="$t('placeholder.input')" maxlength="6" :disabled="emailDisabledEdit" style="width: 70%"></el-input>
    ...
    <el-dialog :title="$t('button.delete')" :visible.sync="deleteDialogVisible" :close-on-click-modal="false">
      <div class="deleteInfo">
        <i class="el-icon-warning"></i><span>{{ $t('message.isDelete') }}</span>
      </div>
      <span slot="footer" class="dialog-footer">
...
export default {
  data() {
    return {
      ...
      coUserId: JSON.parse(sessionStorage.getItem('user')).coUserId, // 是否是当前用户
    ...
    queryFormRules() {
      return {
        searchName: [{
          // trigger: 'blur',
          trigger: 'change',
          message: this.$t('message.noSpecial1'),
          pattern: this.$utils.reg.searchName1,
  ...
  methods: {
    ...
    getList() {
      ...
      this.$http.posts('getCoUser', this.realQuery)
        .then(res => {
          ...
          this.list = (list && list.map((item, index) => {
            var roleName = []
            var roleId = []
            item.roleList && item.roleList.forEach(roleItem => {
              roleName.push(roleItem.roleName)
              roleId.push(roleItem.roleId)
            })
            return {
              ...
              roleId: roleId,
              adminUser: roleId.join() === '1'// 是否是超级管理员
            }
          })) || []
          // } else {
          //   this.$utils.$message({
    ...
    getRoleList(companyId) {
      ...
      var defaultId = user && user.companyId
      this.$http.posts('comAllRoleList', '', { companyId: companyId || defaultId })
        .then(res => {
          // if (res.data.resCode === 0) {
    ...
    handleDialogOpen(type, row) {
      ...
      if (type === 'add') {
        ...
        // console.log(row)
        this.getRoleList(row.companyId)
        ...
        this.currentRow = { ...this.editForm } // 暂存回显的初始数据
        this.phoneDisabledEdit = !!this.editForm.phoneNumb
        this.emailDisabledEdit = !!this.editForm.emailAddr
        this.$nextTick(() => {
    ...
    handleAddOrEdit: $utils.debounce(function (type) {
      this.$refs[`${type}Form`].validate(async valid => {
        if (valid) {
          ...
          this.$http.posts('addOrUpdateCoUser', form, '', false)
            .then(res => {
              this[`${type}DialogVisible`] = false
              var curUserChangePsd = type === 'edit' && form.coUserId === this.coUserId && this.editForm.password !== this.currentRow.password // 是否是当前用户修改密码
              var curUserChangeRole = type === 'edit' && form.coUserId === this.coUserId && form.roleId !== String(this.currentRow.roleId) // 是否是当前用户修改角色
    ...
    handleDelete() {
      // 0 禁用  1解禁 2删除
      let ids = this.selection.map(item => item.coUserId).join()
      this.$http.posts('delCoUser', '', { status: 2, ids }, true)
        .then(res => {
          // if (status === 2) {
          //   // 判断是不是最后一页全删除了
