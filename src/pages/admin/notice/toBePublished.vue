<template>
  <div id="toBepublished">
    ...
    <div class="result clearfix" v-loading="listLoading">
      <el-table class="overflowTable" ref="resultTable" stripe border max-height="640" :data="list" @selection-change="handleSelectionChange" highlight-current-row>
        ...
        <el-table-column :label="$t('msgCenter.msgContent')" min-width="100">
          <div class="moreCell" slot-scope="scope">
...
export default {
  ...
  methods: {
        ...
        .then(res => {
          ...
          this.list = list.map((item, index) => {
            return {
              ...
              // createTime: item.createTime + ' 00:00:00'
            }
          })
        })
        .catch(er => {
    ...
    // 删除
    handleDelete() {
      let ids = this.selection.map(item => item.anncId).toString()
      this.$http.posts('deleteAnnouncement', '', { ids }, true)
        .then(res => {
