      ...
      <div id="scrollView" class="flex-d">
        ...
        <div class="result clearfix">
          <el-table class="overflowTable" ref="resultTable" stripe border max-height="560" :data="tableData" v-loading="loading" tooltip-effect="dark" style="width: 100%;" highlight-current-row>
            ...
            <el-table-column prop="initialAmount" :label="$t('voucher.tab4_1')"></el-table-column>
            ...
            <el-table-column prop="theWay" :label="$t('coupon.recMode')">
              <template slot-scope="scope">
                <span v-if="scope.row.result == 1">手动</span>
                <span v-if="scope.row.result == 2">自动</span>
              </template>
            </el-table-column>
  ...
  data() {
    return {
      ...
      queryForm: {
        ...
        opType: 3, // 操作类型：1.已发放 2.操作记录 3.回收记录
        pageNumber: 1,
        pageSize: 10
      },
      total: 0, // 分页数据，总条数
    ...
    getList() {
      ...
      this.$http.posts('searchPageOp', this.activeQuery)
        .then(res => {
          ...
          let list = data.list
          list.forEach(item => { item.type = '回收抵用券' })
          this.tableData = list
        })
        .catch(er => {
          this.loading = false
