          ...
          <div class="flex">
            ...
            <el-form-item :prop="`pushFormList[${index}].sendTo`" :rules="pushFormRules.sendTo">
              <el-checkbox-group v-model="pushForm.sendTo">
                <el-checkbox :label="1">{{ $t('common.mail') }}</el-checkbox>
                <el-checkbox :label="2">{{ $t('common.shortMessages') }}</el-checkbox>
                <el-checkbox :label="3">{{ $t('msgCenter.backEnd') }}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </div>
          <div class="flex">
      ...
      <el-form-item v-for="(pushForm,index) in pushForm.pushFormList" :key="index">
        <div class="flex-w">
          <div class="flex" :style="{width:width}">
            <template v-if="pushForm.cnfId === 6">
              <span>{{ $t('msgCenter.prodCreateNotice') }}&nbsp;</span>
            </template>
            <template v-else-if="pushForm.cnfId === 7">
              <span>{{ $t('msgCenter.prodResourceRelease') }}&nbsp;</span>
            </template>
            <template v-else>
              <span>{{ $t('msgCenter.timeLimit') }}&nbsp;</span>
              <el-form-item>
          ...
          <div class="flex">
            ...
            <el-form-item :prop="`pushFormList[${index}].sendTo`" :rules="pushFormRules.sendTo">
              <el-checkbox-group v-model="pushForm.sendTo">
                <!-- <el-checkbox :label="1">{{ $t('common.mail') }}</el-checkbox>
                <el-checkbox :label="2" :disabled="true">{{ $t('common.shortMessages') }}</el-checkbox>
                <el-checkbox :label="3">{{ $t('msgCenter.backEnd') }}</el-checkbox> -->
          ...
          <div class="flex">
            <template v-if="pushForm.cnfId === 6">
              <span>{{ $t('msgCenter.sendImmediately1') }}</span>
            </template>
            <template v-else-if="pushForm.cnfId === 7">
...
export default {
  data() {
    return {
      pushForm: {
        pushFormList: [
          { cnfId: 6, deadline: 0, sendTo: [], frequencyDay: '0', frequencyNumb: '1' },
          { deadline: 60, sendTo: [], frequencyDay: '', frequencyNumb: '1' },
          { deadline: 30, sendTo: [], frequencyDay: '', frequencyNumb: '1' },
          { deadline: 15, sendTo: [], frequencyDay: '', frequencyNumb: '1' },
          { deadline: 7, sendTo: [], frequencyDay: '', frequencyNumb: '1' },
          { deadline: 1, sendTo: [], frequencyDay: '', frequencyNumb: '1' },
          { cnfId: 7, deadline: 0, sendTo: [], frequencyDay: '0', frequencyNumb: '1' }
        ]
        // // 如果数据库没有，就是从来没有设置过，采用默认的
        // defaultPushFormList: [
        //   { cnfId: 6, deadline: 0, sendTo: [1, 3], frequencyDay: '0', frequencyNumb: '1' },
        //   { deadline: 60, sendTo: [1, 2, 3], frequencyDay: '10', frequencyNumb: '1' },
        //   { deadline: 30, sendTo: [1, 2, 3], frequencyDay: '5', frequencyNumb: '1' },
        //   { deadline: 15, sendTo: [1, 2, 3], frequencyDay: '5', frequencyNumb: '1' },
        //   { deadline: 7, sendTo: [1, 2, 3], frequencyDay: '2', frequencyNumb: '1' },
        //   { deadline: 1, sendTo: [1, 2, 3], frequencyDay: '1', frequencyNumb: '1' },
        //   { cnfId: 7, deadline: 0, sendTo: [1, 3], frequencyDay: '0', frequencyNumb: '1' }
        // ]
      },
      listLoading: false,
      width: '254px',
