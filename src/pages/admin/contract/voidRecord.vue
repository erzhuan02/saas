    ...
    <div class="result clearfix" v-loading="listLoading">
      <el-table ref="resultTable" stripe border max-height="650" :data="list" highlight-current-row>
        ...
        <el-table-column :label="$t('common.operation')" width="220">
          <template slot-scope="scope">
            <el-button v-if="button.downloadVoid" size="mini" type="primary" @click="handleDownload(scope.row)" plain>{{ $t('button.download') }}</el-button>
            <el-button v-if="button.previewVoid" size="mini" type="primary" @click="handlePreview(scope.row)" plain>{{ $t('button.preview') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
