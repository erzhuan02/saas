<template>
  <div id="dingding">
    ...
    <!-- 操作 -->
    <div class="CRUD">
      <el-button v-if="button.addStream" type="primary" icon="el-icon-plus" @click="handleDialogOpen('add')">{{ $t('button.add') }}</el-button>
      <el-button v-if="button.deleteStream" type="primary" icon="el-icon-delete" @click="handleDialogOpen('delete')" :disabled="selection.length === 0">{{ $t('button.delete') }}</el-button>
    </div>
    <!-- 表格 -->
    <div class="result clearfix" v-loading="listLoading">
      <div class="caption">{{ $t('dingding.list2') }}</div>
      <el-table ref="table" stripe border max-height="565" :data="list" @selection-change="handleSelectionChange" highlight-current-row>
        ...
        <el-table-column prop="dingdingServer" :label="$t('dingding.pushAddress')" min-width="100"></el-table-column>
        <el-table-column :label="$t('common.operation')">
          <template slot-scope="scope">
            <el-button v-if="button.editStream" size="mini" type="primary" icon="el-icon-edit" @click="handleDialogOpen('edit', scope.row)" plain>{{ $t('button.edit') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="$t('button.add')" :visible.sync="addDialogVisible" :close-on-click-modal="false" @closed="handleDialogClose('add')">
      <el-form :model="addForm" ref="addForm" label-width="150px" :rules="addOrEditFormRules" :validate-on-rule-change="false">
        ...
        <el-form-item :label="$t('dingding.pushURL')" prop="dingdingServer">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$http.posts('domainNameList', this.realQuery)
        .then(res => {
    ...
    handleDialogOpen(type, row) {
      if (type === 'add') {
        ...
        this.editForm = { ...row, type: 3, opType: 2 }
        this.$nextTick(() => {
    ...
    handleDelete() {
      ...
      this.$utils.disabledButton(this)
      this.$http.posts('delPlayDomain', '', { companyId: ids }, true)
        .then(res => {
          this.getList()
        })
      this.deleteDialogVisible = false
