<template>
  <div id="userAccount">
    ...
    <div class="CRUD">
      ...
      <el-button v-if="button.deleteUser" type="primary" icon="el-icon-delete" @click="handleDialogOpen('delete')" :disabled="selection.length === 0">{{ $t('button.delete') }}</el-button>
    </div>
    <!-- 查询结果 表格 -->
    <div class="result clearfix" v-loading="listLoading">
      <el-table ref="resultTable" stripe border max-height="565" :data="list" tooltip-effect="dark" style="width: 100%;" @selection-change="handleSelectionChange" @sort-change="handleSortChange" highlight-current-row>
        ...
        <el-table-column sortable="custom" prop="lastLoginTime" :label="$t('account.lastLoginTime')" width="170"></el-table-column>
    ...
    <el-dialog :title="$t('button.add')" :visible.sync="addDialogVisible" :close-on-click-modal="false" @closed="handleDialogClose('add')">
      <el-form :model="addForm" label-width="130px" :rules="addOrEditFormRules" ref="addForm" :validate-on-rule-change="false">
        ...
        <el-form-item prop="emailAddr" :label="$t('common.email')">
          <el-input v-model.trim="addForm.emailAddr" :placeholder="$t('placeholder.input')" maxlength="50" style="width: 100%;"></el-input>
        </el-form-item>
...
export default {
        ...
        emailAddr: [
          {
            ...
            trigger: 'blur',
            validator: (rule, value, callback) => {
              let emailReg = this.$utils.reg.email
              if (value === '') {
  ...
  methods: {
    ...
    handleDialogOpen(type, row) {
      ...
      if (type === 'add') {
        this.$store.dispatch('isHasRole', 1).then(res => {
        ...
        this.editForm = { ...row, phonePrefix, phoneNumb, vailCode: '', emailCode: '', type: 2 }
    ...
    handleDelete: $utils.debounce(function () {
      // 0 禁用  1解禁 2删除
      let ids = this.selection.map((item, index) => item.coUserId).toString()
