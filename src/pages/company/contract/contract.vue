<template>
  <div id="contractMgt">
    ...
    <!-- 操作 -->
    <div class="CRUD">
      <el-button v-if="button.applyContr" type="primary" @click="handleApply">{{ $t('button.applyContr') }}</el-button>
    </div>
    <!-- 查询结果 表格 -->
    <div class="result clearfix" v-loading="listLoading">
      ...
      <el-table ref="resultTable" stripe border max-height="650" :data="list" highlight-current-row>
        ...
        <el-table-column prop="contractId" :label="$t('contract.contractNo')" min-width="110"></el-table-column>
        ...
        <el-table-column prop="createTime" :label="$t('contract.applyTime')" min-width="90"></el-table-column>
        ...
        <el-table-column :label="$t('common.operation')" width="420">
          <template slot-scope="scope">
            <el-button v-if="button.download" size="mini" type="primary" @click="handleDownload (scope.row)" plain>{{ $t('button.download') }}</el-button>
            <el-button v-if="button.toFormal && scope.row.contractStatus === 1" size="mini" type="primary" @click="handleDialogOpen ('toFormal', scope.row)" plain>{{ $t('button.toFormal') }}</el-button>
            <el-button v-if="button.void && scope.row.contractStatus === 1" size="mini" type="primary" @click="handleDialogOpen ('void', scope.row)" plain>{{ $t('button.void') }}</el-button>
            <el-button v-if="button.delete && scope.row.contractStatus === 3" size="mini" type="primary" @click="handleDialogOpen ('delete', scope.row)" plain>{{ $t('button.delete') }}</el-button>
            <el-button v-if="button.preview" size="mini" type="primary" @click="handlePreview(scope.row)" plain>{{ $t('button.preview') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 转正式合同对话框 -->
    <el-dialog :title="$t('button.toFormal')" :visible.sync="toFormalDialogVisible" :close-on-click-modal="false">
      <div class="dialogInfo flex-vc" ref="toFormalBody">
        <i class="el-icon-warning"></i>
        <div>
          <div>{{ $t('message.toFormalContract1') }}</div>
          <div>{{ $t('message.toFormalContract') }}</div>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="toFormalDialogVisible = false">{{ $t('button.cancel') }}</el-button>
        <el-button type="primary" @click="handleToFormal('toFormal',2)">{{ $t('button.confirm') }}</el-button>
      </span>
    </el-dialog>
    <!-- 作废对话框 -->
    ...
    <el-dialog :title="$t('button.delete')" :visible.sync="deleteDialogVisible" :close-on-click-modal="false">
      <div class="dialogInfo" ref="deleteBody">
        <i class="el-icon-warning"></i><span>{{ $t('message.deleteContract') }}</span>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="deleteDialogVisible = false">{{ $t('button.cancel') }}</el-button>
        <el-button type="primary" @click="handleUpdateStatus('delete',0)">{{ $t('button.confirm') }}</el-button>
      </span>
    </el-dialog>
    <!-- <router-link to="/solution/1">shish</router-link> -->
...
export default {
  ...
  methods: {
    ...
    // 申请合同
    handleApply() {
      this.$router.push({ path: '/comContractApply' })
    },
    // 下载
    ...
    // 新增/转正式合同
    handleToFormal: $utils.debounce(function (type, contractStatus) {
      // type add新增 edit修改转正式
      // contractStatus合同状态 1草稿 2正式
      var form = { contractStatus }
      // if (type === 'edit') {
      form.contractId = this.currentRow.contractId
      form.type = 2 // 创建合同类型 2正式
      // }
      // this.listLoading = true
      this.$http.posts('addOrUpdateContr', form, '', { showSuccessMsg: true, showErrorMsg: false })
        .then(res => {
          // this.listLoading = false
          this.getList()
        })
      ...
      this.toFormalDialogVisible = false
    }),
    // 删除和改变状态
