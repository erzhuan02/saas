  ...
  <div id="assoFirst">
    <div class="result clearfix" v-loading="listLoading">
      <el-table class="overflowTable" ref="table" stripe border max-height="565" :data="list" highlight-current-row>
        ...
        <el-table-column prop="updateTime" :label="$t('association.authTime')" min-width="100"></el-table-column>
        ...
        <el-table-column :label="$t('common.operation')" min-width="100">
          <template slot-scope="scope">
            <el-button v-if="button.disconnect" size="mini" type="primary" @click="handleDialogOpen('disconnect', scope.row)" plain>{{ $t('button.disconnect') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="$t('dialog.tips')" :visible.sync="disconnectDialogVisible" :close-on-click-modal="false">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleUpdateStatus('disconnect',0)" :disabled="buttonDisabled">{{ $t('button.confirm') }}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
