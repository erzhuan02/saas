    ...
    <el-container class="flex1">
      ...
      <el-aside class="indexAside" width="auto">
        <el-menu :default-active="$route.path" :router="true" :collapse="isCollapse" :unique-opened="true" background-color="#222" text-color="#fff">
          <template v-for="(item, index) in visualSafeMenuTree">
...
export default {
  components: { Lang, LogOut, BusiModule },
  data() {
    return {
      isCollapse: false, // 菜单的显示和隐藏
      visualSafeMenuTree: [] // 左侧菜单-视安心
    }
  },
  mounted() {
    this.visualSafeMenuTree = JSON.parse(sessionStorage.getItem('visualSafeLeftTree')) || []
  },
